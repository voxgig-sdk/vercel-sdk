...
describe("KmsDirect", function()
  it("should direct-list-kms", function()
    local setup = kms_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-kms", setup.live and "live" or "unit")
  ...
  it("should direct-load-kms", function()
    local setup = kms_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-kms", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function kms_direct_setup(mockres)
