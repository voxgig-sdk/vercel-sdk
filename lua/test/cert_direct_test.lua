...
describe("CertDirect", function()
  it("should direct-list-cert", function()
    local setup = cert_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-cert", setup.live and "live" or "unit")
  ...
  it("should direct-load-cert", function()
    local setup = cert_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-cert", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function cert_direct_setup(mockres)
