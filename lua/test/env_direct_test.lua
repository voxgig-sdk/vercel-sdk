...
describe("EnvDirect", function()
  it("should direct-list-env", function()
    local setup = env_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-env", setup.live and "live" or "unit")
  ...
  it("should direct-load-env", function()
    local setup = env_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-env", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function env_direct_setup(mockres)
