-- ConnectConnectorProjectConnectionList direct test
...
describe("ConnectConnectorProjectConnectionListDirect", function()
  it("should direct-list-connect_connector_project_connection_list", function()
    local setup = connect_connector_project_connection_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-connect_connector_project_connection_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"connector01"}) do
...
function connect_connector_project_connection_list_direct_setup(mockres)
