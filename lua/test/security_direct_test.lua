...
describe("SecurityDirect", function()
  it("should direct-list-security", function()
    local setup = security_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-security", setup.live and "live" or "unit")
  ...
  it("should direct-load-security", function()
    local setup = security_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-security", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function security_direct_setup(mockres)
