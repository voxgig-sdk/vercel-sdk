...
describe("DeploymentEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_DEPLOYMENT_ENTID JSON to run live")
      return
    end
    ...
    -- UPDATE
    local deployment_ref01_data_up0_up = {
      id = deployment_ref01_data["id"],
    }

    local deployment_ref01_markdef_up0_name = "buildMachine"
    local deployment_ref01_markdef_up0_value = "Mark01-deployment_ref01_" .. tostring(setup.now)
    deployment_ref01_data_up0_up[deployment_ref01_markdef_up0_name] = deployment_ref01_markdef_up0_value

    local deployment_ref01_resdata_up0_result, err = deployment_ref01_ent:update(deployment_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local deployment_ref01_resdata_up0 = helpers.to_map(type(deployment_ref01_resdata_up0_result) == 'table' and deployment_ref01_resdata_up0_result.data_get and deployment_ref01_resdata_up0_result:data_get() or deployment_ref01_resdata_up0_result)
    assert.is_not_nil(deployment_ref01_resdata_up0)
    assert.are.equal(deployment_ref01_resdata_up0["id"], deployment_ref01_data_up0_up["id"])
    assert.are.equal(deployment_ref01_resdata_up0[deployment_ref01_markdef_up0_name], deployment_ref01_markdef_up0_value)
    ...
    -- LIST
    local deployment_ref01_match_rt0 = {
      ["id_or_url"] = setup.idmap["id_or_url01"],
    }

    local deployment_ref01_list_rt0_result, err = deployment_ref01_ent:list(deployment_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(deployment_ref01_list_rt0_result)
...
function deployment_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("VERCEL_TEST_DEPLOYMENT_ENTID")
