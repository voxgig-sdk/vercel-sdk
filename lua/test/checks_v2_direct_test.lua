...
describe("ChecksV2Direct", function()
  it("should direct-list-checks_v2", function()
    local setup = checks_v2_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-checks_v2", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"check01", "project01"}) do
  ...
  it("should direct-load-checks_v2", function()
    local setup = checks_v2_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-checks_v2", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function checks_v2_direct_setup(mockres)
