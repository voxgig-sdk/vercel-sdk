...
describe("AliasDirect", function()
  it("should direct-list-alias", function()
    local setup = alias_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-alias", setup.live and "live" or "unit")
  ...
  it("should direct-load-alias", function()
    local setup = alias_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-alias", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function alias_direct_setup(mockres)
