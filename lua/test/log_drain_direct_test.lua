...
describe("LogDrainDirect", function()
  it("should direct-list-log_drain", function()
    local setup = log_drain_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-log_drain", setup.live and "live" or "unit")
  ...
  it("should direct-load-log_drain", function()
    local setup = log_drain_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-log_drain", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function log_drain_direct_setup(mockres)
