...
describe("MicrofrontendDirect", function()
  it("should direct-list-microfrontend", function()
    local setup = microfrontend_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-microfrontend", setup.live and "live" or "unit")
  ...
  it("should direct-load-microfrontend", function()
    local setup = microfrontend_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-microfrontend", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function microfrontend_direct_setup(mockres)
