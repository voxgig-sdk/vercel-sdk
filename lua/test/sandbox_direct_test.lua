...
describe("SandboxDirect", function()
  it("should direct-list-sandbox", function()
    local setup = sandbox_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sandbox", setup.live and "live" or "unit")
  ...
  it("should direct-load-sandbox", function()
    local setup = sandbox_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-sandbox", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function sandbox_direct_setup(mockres)
