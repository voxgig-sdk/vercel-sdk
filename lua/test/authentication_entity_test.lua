...
describe("AuthenticationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_AUTHENTICATION_ENTID JSON to run live")
      return
    end
    ...
    -- REMOVE
    local authentication_ref01_match_rm0 = {
      id = authentication_ref01_data["id"],
    }
    local _, err = authentication_ref01_ent:remove(authentication_ref01_match_rm0, nil)
    assert.is_nil(err)
...
function authentication_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("VERCEL_TEST_AUTHENTICATION_ENTID")
