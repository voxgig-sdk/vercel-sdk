...
local EnvEntity = {}
EnvEntity.__index = EnvEntity


function EnvEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EnvEntity)
  self._name = "env"
  self._client = client
...
function EnvEntity:get_name()
  return self._name
end


function EnvEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EnvEntity.new(self._client, opts)
end
...
function EnvEntity:mark_deleted()
  self._deleted = true
end


function EnvEntity:deleted()
  return true == self._deleted
end


function EnvEntity:data_set(args)
  if args ~= nil then
...
function EnvEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EnvEntity:match_set(args)
  if args ~= nil then
...
function EnvEntity:stream(action, args, callopts)
...
---@param reqmatch EnvLoadMatch
---@param ctrl? table
---@return Env
---@return string? err
function EnvEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EnvListMatch
---@param ctrl? table
---@return Env[]
---@return string? err
function EnvEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata EnvCreateData
---@param ctrl? table
---@return Env
---@return string? err
function EnvEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata EnvUpdateData
---@param ctrl? table
---@return Env
---@return string? err
function EnvEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch EnvRemoveMatch
---@param ctrl? table
---@return Env
---@return string? err
function EnvEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EnvEntity:_run_op(ctx, post_done)
  local utility = self._utility
