...
local ApiAiGatewayEntity = {}
ApiAiGatewayEntity.__index = ApiAiGatewayEntity


function ApiAiGatewayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiAiGatewayEntity)
  self._name = "api_ai_gateway"
  self._client = client
...
function ApiAiGatewayEntity:get_name()
  return self._name
end


function ApiAiGatewayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiAiGatewayEntity.new(self._client, opts)
end
...
function ApiAiGatewayEntity:mark_deleted()
  self._deleted = true
end


function ApiAiGatewayEntity:deleted()
  return true == self._deleted
end


function ApiAiGatewayEntity:data_set(args)
  if args ~= nil then
...
function ApiAiGatewayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiAiGatewayEntity:match_set(args)
  if args ~= nil then
...
function ApiAiGatewayEntity:match_get()
...
function ApiAiGatewayEntity:stream(action, args, callopts)
...
---@param reqmatch ApiAiGatewayLoadMatch
---@param ctrl? table
---@return ApiAiGateway
---@return string? err
function ApiAiGatewayEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ApiAiGatewayRemoveMatch
---@param ctrl? table
---@return ApiAiGateway
---@return string? err
function ApiAiGatewayEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ApiAiGatewayEntity:_run_op(ctx, post_done)
  local utility = self._utility
