...
local NetworkingEntity = {}
NetworkingEntity.__index = NetworkingEntity


function NetworkingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NetworkingEntity)
  self._name = "networking"
  self._client = client
...
function NetworkingEntity:get_name()
  return self._name
end


function NetworkingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NetworkingEntity.new(self._client, opts)
end
...
function NetworkingEntity:mark_deleted()
  self._deleted = true
end


function NetworkingEntity:deleted()
  return true == self._deleted
end


function NetworkingEntity:data_set(args)
  if args ~= nil then
...
function NetworkingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NetworkingEntity:match_set(args)
  if args ~= nil then
...
function NetworkingEntity:match_get()
...
function NetworkingEntity:stream(action, args, callopts)
...
---@param reqdata NetworkingUpdateData
---@param ctrl? table
---@return Networking
---@return string? err
function NetworkingEntity:update(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch NetworkingRemoveMatch
---@param ctrl? table
---@return Networking
---@return string? err
function NetworkingEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function NetworkingEntity:_run_op(ctx, post_done)
  local utility = self._utility
