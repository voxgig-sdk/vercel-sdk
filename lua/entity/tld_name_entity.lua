-- Vercel SDK TldName entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TldNameEntity = {}
TldNameEntity.__index = TldNameEntity


function TldNameEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TldNameEntity)
  self._name = "tld_name"
  self._client = client
...
function TldNameEntity:get_name()
  return self._name
end


function TldNameEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TldNameEntity.new(self._client, opts)
end
...
function TldNameEntity:mark_deleted()
  self._deleted = true
end


function TldNameEntity:deleted()
  return true == self._deleted
end


function TldNameEntity:data_set(args)
  if args ~= nil then
...
function TldNameEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TldNameEntity:match_set(args)
  if args ~= nil then
...
function TldNameEntity:match_get()
...
function TldNameEntity:stream(action, args, callopts)
...
---@param reqmatch TldNameListMatch
---@param ctrl? table
---@return TldName[]
---@return string? err
function TldNameEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TldNameEntity:_run_op(ctx, post_done)
  local utility = self._utility
