...
local DrainEntity = {}
DrainEntity.__index = DrainEntity


function DrainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DrainEntity)
  self._name = "drain"
  self._client = client
...
function DrainEntity:get_name()
  return self._name
end


function DrainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DrainEntity.new(self._client, opts)
end
...
function DrainEntity:mark_deleted()
  self._deleted = true
end


function DrainEntity:deleted()
  return true == self._deleted
end


function DrainEntity:data_set(args)
  if args ~= nil then
...
function DrainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DrainEntity:match_set(args)
  if args ~= nil then
...
function DrainEntity:match_get()
...
function DrainEntity:stream(action, args, callopts)
...
---@param reqmatch DrainLoadMatch
---@param ctrl? table
---@return Drain
---@return string? err
function DrainEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DrainCreateData
---@param ctrl? table
---@return Drain
---@return string? err
function DrainEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata DrainUpdateData
---@param ctrl? table
---@return Drain
---@return string? err
function DrainEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch DrainRemoveMatch
---@param ctrl? table
---@return Drain
---@return string? err
function DrainEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function DrainEntity:_run_op(ctx, post_done)
  local utility = self._utility
