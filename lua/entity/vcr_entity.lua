...
local VcrEntity = {}
VcrEntity.__index = VcrEntity


function VcrEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VcrEntity)
  self._name = "vcr"
  self._client = client
...
function VcrEntity:get_name()
  return self._name
end


function VcrEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VcrEntity.new(self._client, opts)
end
...
function VcrEntity:mark_deleted()
  self._deleted = true
end


function VcrEntity:deleted()
  return true == self._deleted
end


function VcrEntity:data_set(args)
  if args ~= nil then
...
function VcrEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VcrEntity:match_set(args)
  if args ~= nil then
...
function VcrEntity:stream(action, args, callopts)
...
---@param reqmatch VcrLoadMatch
---@param ctrl? table
---@return Vcr
---@return string? err
function VcrEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch VcrListMatch
---@param ctrl? table
---@return Vcr[]
---@return string? err
function VcrEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata VcrCreateData
---@param ctrl? table
---@return Vcr
---@return string? err
function VcrEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata VcrUpdateData
---@param ctrl? table
---@return Vcr
---@return string? err
function VcrEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch VcrRemoveMatch
---@param ctrl? table
---@return Vcr
---@return string? err
function VcrEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function VcrEntity:_run_op(ctx, post_done)
  local utility = self._utility
