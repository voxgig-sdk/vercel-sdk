-- Vercel SDK AiGatewayRule entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AiGatewayRuleEntity = {}
AiGatewayRuleEntity.__index = AiGatewayRuleEntity


function AiGatewayRuleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiGatewayRuleEntity)
  self._name = "ai_gateway_rule"
  self._client = client
...
function AiGatewayRuleEntity:get_name()
  return self._name
end


function AiGatewayRuleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiGatewayRuleEntity.new(self._client, opts)
end
...
function AiGatewayRuleEntity:mark_deleted()
  self._deleted = true
end


function AiGatewayRuleEntity:deleted()
  return true == self._deleted
end


function AiGatewayRuleEntity:data_set(args)
  if args ~= nil then
...
function AiGatewayRuleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiGatewayRuleEntity:match_set(args)
  if args ~= nil then
...
function AiGatewayRuleEntity:match_get()
...
function AiGatewayRuleEntity:stream(action, args, callopts)
...
---@param reqdata AiGatewayRuleCreateData
---@param ctrl? table
---@return AiGatewayRule
---@return string? err
function AiGatewayRuleEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqdata AiGatewayRuleUpdateData
---@param ctrl? table
---@return AiGatewayRule
---@return string? err
function AiGatewayRuleEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AiGatewayRuleEntity:_run_op(ctx, post_done)
  local utility = self._utility
