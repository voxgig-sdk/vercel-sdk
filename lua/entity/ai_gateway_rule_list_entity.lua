-- Vercel SDK AiGatewayRuleList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AiGatewayRuleListEntity = {}
AiGatewayRuleListEntity.__index = AiGatewayRuleListEntity


function AiGatewayRuleListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiGatewayRuleListEntity)
  self._name = "ai_gateway_rule_list"
  self._client = client
...
function AiGatewayRuleListEntity:get_name()
  return self._name
end


function AiGatewayRuleListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiGatewayRuleListEntity.new(self._client, opts)
end
...
function AiGatewayRuleListEntity:mark_deleted()
  self._deleted = true
end


function AiGatewayRuleListEntity:deleted()
  return true == self._deleted
end


function AiGatewayRuleListEntity:data_set(args)
  if args ~= nil then
...
function AiGatewayRuleListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiGatewayRuleListEntity:match_set(args)
  if args ~= nil then
...
function AiGatewayRuleListEntity:match_get()
...
function AiGatewayRuleListEntity:stream(action, args, callopts)
...
---@param reqmatch AiGatewayRuleListListMatch
---@param ctrl? table
---@return AiGatewayRuleList[]
---@return string? err
function AiGatewayRuleListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AiGatewayRuleListEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AiGatewayRuleListEntity
