...
local GlobalConfigEntity = {}
GlobalConfigEntity.__index = GlobalConfigEntity


function GlobalConfigEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GlobalConfigEntity)
...
function GlobalConfigEntity:get_name()
  return self._name
end


function GlobalConfigEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GlobalConfigEntity.new(self._client, opts)
end
...
function GlobalConfigEntity:mark_deleted()
  self._deleted = true
end


function GlobalConfigEntity:deleted()
  return true == self._deleted
end


function GlobalConfigEntity:data_set(args)
  if args ~= nil then
...
function GlobalConfigEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GlobalConfigEntity:match_set(args)
  if args ~= nil then
...
function GlobalConfigEntity:match_get()
...
function GlobalConfigEntity:stream(action, args, callopts)
...
---@param reqmatch GlobalConfigLoadMatch
---@param ctrl? table
---@return GlobalConfig
---@return string? err
function GlobalConfigEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GlobalConfigListMatch
---@param ctrl? table
---@return GlobalConfig[]
---@return string? err
function GlobalConfigEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata GlobalConfigCreateData
---@param ctrl? table
---@return GlobalConfig
---@return string? err
function GlobalConfigEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata GlobalConfigUpdateData
---@param ctrl? table
---@return GlobalConfig
---@return string? err
function GlobalConfigEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch GlobalConfigRemoveMatch
---@param ctrl? table
---@return GlobalConfig
---@return string? err
function GlobalConfigEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function GlobalConfigEntity:_run_op(ctx, post_done)
  local utility = self._utility
