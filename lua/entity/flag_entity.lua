-- Vercel SDK Flag entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FlagEntity = {}
FlagEntity.__index = FlagEntity


function FlagEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FlagEntity)
  self._name = "flag"
  self._client = client
...
function FlagEntity:get_name()
  return self._name
end


function FlagEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FlagEntity.new(self._client, opts)
end
...
function FlagEntity:mark_deleted()
  self._deleted = true
end


function FlagEntity:deleted()
  return true == self._deleted
end


function FlagEntity:data_set(args)
  if args ~= nil then
...
function FlagEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FlagEntity:match_set(args)
  if args ~= nil then
...
function FlagEntity:stream(action, args, callopts)
...
---@param reqmatch FlagLoadMatch
---@param ctrl? table
---@return Flag
---@return string? err
function FlagEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FlagEntity:_run_op(ctx, post_done)
  local utility = self._utility
