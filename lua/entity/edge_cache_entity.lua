-- Vercel SDK EdgeCache entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EdgeCacheEntity = {}
EdgeCacheEntity.__index = EdgeCacheEntity


function EdgeCacheEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EdgeCacheEntity)
  self._name = "edge_cache"
  self._client = client
...
function EdgeCacheEntity:get_name()
  return self._name
end


function EdgeCacheEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EdgeCacheEntity.new(self._client, opts)
end
...
function EdgeCacheEntity:mark_deleted()
  self._deleted = true
end


function EdgeCacheEntity:deleted()
  return true == self._deleted
end


function EdgeCacheEntity:data_set(args)
  if args ~= nil then
...
function EdgeCacheEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EdgeCacheEntity:match_set(args)
  if args ~= nil then
...
function EdgeCacheEntity:match_get()
...
function EdgeCacheEntity:stream(action, args, callopts)
...
---@param reqdata EdgeCacheCreateData
---@param ctrl? table
---@return EdgeCache
---@return string? err
function EdgeCacheEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EdgeCacheEntity:_run_op(ctx, post_done)
  local utility = self._utility
