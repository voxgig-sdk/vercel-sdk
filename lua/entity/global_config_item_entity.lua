...
local GlobalConfigItemEntity = {}
GlobalConfigItemEntity.__index = GlobalConfigItemEntity


function GlobalConfigItemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GlobalConfigItemEntity)
  self._name = "global_config_item"
  self._client = client
...
function GlobalConfigItemEntity:get_name()
  return self._name
end


function GlobalConfigItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GlobalConfigItemEntity.new(self._client, opts)
end
...
function GlobalConfigItemEntity:mark_deleted()
  self._deleted = true
end


function GlobalConfigItemEntity:deleted()
  return true == self._deleted
end


function GlobalConfigItemEntity:data_set(args)
  if args ~= nil then
...
function GlobalConfigItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GlobalConfigItemEntity:match_set(args)
  if args ~= nil then
...
function GlobalConfigItemEntity:match_get()
...
function GlobalConfigItemEntity:stream(action, args, callopts)
...
---@param reqmatch GlobalConfigItemLoadMatch
---@param ctrl? table
---@return GlobalConfigItem
---@return string? err
function GlobalConfigItemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GlobalConfigItemListMatch
---@param ctrl? table
---@return GlobalConfigItem[]
---@return string? err
function GlobalConfigItemEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function GlobalConfigItemEntity:_run_op(ctx, post_done)
  local utility = self._utility
