-- Vercel SDK AiGateway entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AiGatewayEntity = {}
AiGatewayEntity.__index = AiGatewayEntity


function AiGatewayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiGatewayEntity)
  self._name = "ai_gateway"
  self._client = client
...
function AiGatewayEntity:get_name()
  return self._name
end


function AiGatewayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiGatewayEntity.new(self._client, opts)
end
...
function AiGatewayEntity:mark_deleted()
  self._deleted = true
end


function AiGatewayEntity:deleted()
  return true == self._deleted
end


function AiGatewayEntity:data_set(args)
  if args ~= nil then
...
function AiGatewayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiGatewayEntity:match_set(args)
  if args ~= nil then
...
function AiGatewayEntity:match_get()
...
function AiGatewayEntity:stream(action, args, callopts)
...
---@param reqmatch AiGatewayRemoveMatch
---@param ctrl? table
---@return AiGateway
---@return string? err
function AiGatewayEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function AiGatewayEntity:_run_op(ctx, post_done)
  local utility = self._utility
