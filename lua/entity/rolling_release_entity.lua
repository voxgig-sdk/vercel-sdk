...
local RollingReleaseEntity = {}
RollingReleaseEntity.__index = RollingReleaseEntity


function RollingReleaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RollingReleaseEntity)
  self._name = "rolling_release"
  self._client = client
...
function RollingReleaseEntity:get_name()
  return self._name
end


function RollingReleaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RollingReleaseEntity.new(self._client, opts)
end
...
function RollingReleaseEntity:mark_deleted()
  self._deleted = true
end


function RollingReleaseEntity:deleted()
  return true == self._deleted
end


function RollingReleaseEntity:data_set(args)
  if args ~= nil then
...
function RollingReleaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RollingReleaseEntity:match_set(args)
  if args ~= nil then
...
function RollingReleaseEntity:match_get()
...
function RollingReleaseEntity:stream(action, args, callopts)
...
---@param reqmatch RollingReleaseLoadMatch
---@param ctrl? table
---@return RollingRelease
---@return string? err
function RollingReleaseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RollingReleaseCreateData
---@param ctrl? table
---@return RollingRelease
---@return string? err
function RollingReleaseEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata RollingReleaseUpdateData
---@param ctrl? table
---@return RollingRelease
---@return string? err
function RollingReleaseEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch RollingReleaseRemoveMatch
---@param ctrl? table
---@return RollingRelease
---@return string? err
function RollingReleaseEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function RollingReleaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
