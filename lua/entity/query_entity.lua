...
function QueryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QueryEntity)
  self._name = "query"
  self._client = client
...
function QueryEntity:get_name()
  return self._name
end


function QueryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QueryEntity.new(self._client, opts)
end
...
function QueryEntity:mark_deleted()
  self._deleted = true
end


function QueryEntity:deleted()
  return true == self._deleted
end


function QueryEntity:data_set(args)
  if args ~= nil then
...
function QueryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QueryEntity:match_set(args)
  if args ~= nil then
...
function QueryEntity:match_get()
...
function QueryEntity:stream(action, args, callopts)
...
---@param reqdata QueryCreateData
---@param ctrl? table
---@return Query
---@return string? err
function QueryEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function QueryEntity:_run_op(ctx, post_done)
  local utility = self._utility
