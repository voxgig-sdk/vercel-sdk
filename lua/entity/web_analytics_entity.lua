-- Vercel SDK WebAnalytics entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WebAnalyticsEntity = {}
WebAnalyticsEntity.__index = WebAnalyticsEntity


function WebAnalyticsEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebAnalyticsEntity)
  self._name = "web_analytics"
  self._client = client
...
function WebAnalyticsEntity:get_name()
  return self._name
end


function WebAnalyticsEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebAnalyticsEntity.new(self._client, opts)
end
...
function WebAnalyticsEntity:mark_deleted()
  self._deleted = true
end


function WebAnalyticsEntity:deleted()
  return true == self._deleted
end


function WebAnalyticsEntity:data_set(args)
  if args ~= nil then
...
function WebAnalyticsEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebAnalyticsEntity:match_set(args)
  if args ~= nil then
...
function WebAnalyticsEntity:match_get()
...
function WebAnalyticsEntity:stream(action, args, callopts)
...
---@param reqmatch WebAnalyticsLoadMatch
---@param ctrl? table
---@return WebAnalytics
---@return string? err
function WebAnalyticsEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WebAnalyticsEntity:_run_op(ctx, post_done)
  local utility = self._utility
