-- Vercel SDK GlobalConfigToken entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GlobalConfigTokenEntity = {}
GlobalConfigTokenEntity.__index = GlobalConfigTokenEntity


function GlobalConfigTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GlobalConfigTokenEntity)
  self._name = "global_config_token"
  self._client = client
...
function GlobalConfigTokenEntity:get_name()
  return self._name
end


function GlobalConfigTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GlobalConfigTokenEntity.new(self._client, opts)
end
...
function GlobalConfigTokenEntity:mark_deleted()
  self._deleted = true
end


function GlobalConfigTokenEntity:deleted()
  return true == self._deleted
end


function GlobalConfigTokenEntity:data_set(args)
  if args ~= nil then
...
function GlobalConfigTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GlobalConfigTokenEntity:match_set(args)
  if args ~= nil then
...
function GlobalConfigTokenEntity:match_get()
...
function GlobalConfigTokenEntity:stream(action, args, callopts)
...
---@param reqmatch GlobalConfigTokenLoadMatch
---@param ctrl? table
---@return GlobalConfigToken
---@return string? err
function GlobalConfigTokenEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GlobalConfigTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return GlobalConfigTokenEntity
