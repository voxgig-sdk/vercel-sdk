...
function StorageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StorageEntity)
  self._name = "storage"
  self._client = client
...
function StorageEntity:get_name()
  return self._name
end


function StorageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StorageEntity.new(self._client, opts)
end
...
function StorageEntity:mark_deleted()
  self._deleted = true
end


function StorageEntity:deleted()
  return true == self._deleted
end


function StorageEntity:data_set(args)
  if args ~= nil then
...
function StorageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StorageEntity:match_set(args)
  if args ~= nil then
...
function StorageEntity:match_get()
...
function StorageEntity:stream(action, args, callopts)
...
---@param reqmatch StorageLoadMatch
---@param ctrl? table
---@return Storage
---@return string? err
function StorageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata StorageCreateData
---@param ctrl? table
---@return Storage
---@return string? err
function StorageEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch StorageRemoveMatch
---@param ctrl? table
---@return Storage
---@return string? err
function StorageEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function StorageEntity:_run_op(ctx, post_done)
  local utility = self._utility
