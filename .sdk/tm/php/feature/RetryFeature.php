...
class VercelRetryFeature extends VercelBaseFeature
{
    ...
    public function init(VercelContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (VercelContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(VercelContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(VercelContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
