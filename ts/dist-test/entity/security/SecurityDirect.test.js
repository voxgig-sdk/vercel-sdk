...
(0, node_test_1.describe)('SecurityDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-security', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-security', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["config_version01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.config_version ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.config_version = candidateId;
    ...
    (0, node_test_1.test)('direct-list-security', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-security', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['VERCEL_TEST_SECURITY_ENTID'];
...
//# sourceMappingURL=SecurityDirect.test.js.map