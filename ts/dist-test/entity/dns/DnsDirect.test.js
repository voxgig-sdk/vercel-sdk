...
(0, node_test_1.describe)('DnsDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-dns', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-dns', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.domain_id = "example.com";
        }
        else {
            params.domain_id = 'direct01';
        }
        const result = await client.direct({
            path: 'v5/domains/{domain_id}/records',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['VERCEL_TEST_DNS_ENTID'];
...
//# sourceMappingURL=DnsDirect.test.js.map