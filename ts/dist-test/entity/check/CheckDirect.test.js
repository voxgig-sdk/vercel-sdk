...
(0, node_test_1.describe)('CheckDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-check', async (t) => {
        ...
        const query = {};
        if (setup.live) {
            params.deployment_id = "dpl_2qn7PZrx89yxY34vEZPD31Y9XVj6";
            params.id = "check_2qn7PZrx89yxY34vEZPD31Y9XVj6";
        }
        else {
        ...
        const result = await client.direct({
            path: 'v1/deployments/{deployment_id}/checks/{id}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-check', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-check', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["project_id_or_name01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.project_id_or_name = setup.idmap['project_id_or_name01'];
        }
        else {
            params.project_id_or_name = 'direct01';
        }
        const result = await client.direct({
            path: 'v2/projects/{project_id_or_name}/checks',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['VERCEL_TEST_CHECK_ENTID'];
