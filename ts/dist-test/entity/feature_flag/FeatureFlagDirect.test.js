...
(0, node_test_1.describe)('FeatureFlagDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-feature_flag', async (t) => {
        ...
        const query = {};
        if (setup.live) {
            params.team_id = "team_1a2b3c4d5e6f7g8h9i0j1k2l";
        }
        else {
            params.team_id = 'direct01';
        }
        const result = await client.direct({
            path: 'v1/teams/{team_id}/feature-flags/settings',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-feature_flag', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-feature_flag', setup.live))
            return;
        ...
        const result = await client.direct({
            path: 'v2/teams/{team_id}/feature-flags/flags',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['VERCEL_TEST_FEATURE_FLAG_ENTID'];
