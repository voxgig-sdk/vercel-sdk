...
(0, node_test_1.describe)('IntegrationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-integration', async (t) => {
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'v1/integrations/integration/{integration_id}/products/{product_id_or_slug}/plans',
                method: 'GET',
                params: {
                    integration_id: setup.idmap['integration01'],
                    product_id_or_slug: setup.idmap['product_id_or_slug01'],
                },
            });
    ...
    (0, node_test_1.test)('direct-list-integration', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-integration', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["integration01", "product_id_or_slug01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.integration_id = setup.idmap['integration01'];
            params.product_id_or_slug = setup.idmap['product_id_or_slug01'];
        }
        else {
            params.integration_id = 'direct01';
            params.product_id_or_slug = 'direct02';
        }
        const result = await client.direct({
            path: 'v1/integrations/integration/{integration_id}/products/{product_id_or_slug}/plans',
            method: 'GET',
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['VERCEL_TEST_INTEGRATION_ENTID'];
