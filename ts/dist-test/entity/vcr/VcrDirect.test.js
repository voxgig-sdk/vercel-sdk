...
(0, node_test_1.describe)('VcrDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-vcr', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-vcr', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["image_id_or_digest01", "repository01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.image_id_or_digest ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.image_id_or_digest = candidateId;
    ...
    (0, node_test_1.test)('direct-list-vcr', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-vcr', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["id_or_name01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['VERCEL_TEST_VCR_ENTID'];
...
//# sourceMappingURL=VcrDirect.test.js.map