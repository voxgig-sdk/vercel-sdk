...
(0, node_test_1.describe)('ChecksV2Direct', async () => {
    ...
    (0, node_test_1.test)('direct-load-checks_v2', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-checks_v2', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["check_run01", "deployment01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.check_run_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.check_run_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-checks_v2', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-checks_v2', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["check01", "project01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
//# sourceMappingURL=ChecksV2Direct.test.js.map