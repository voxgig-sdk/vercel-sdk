...
describe('VcrDirect', async () => {
  ...
  test('direct-load-vcr', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-vcr', setup.live)) return
    if (skipIfMissingIds(t, setup, ["image_id_or_digest01","repository01"])) return
  ...
  test('direct-list-vcr', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-vcr', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id_or_name01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_VCR_ENTID']
