...
describe('VcrRepositoryPermissionListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let vcr_repository_permission_list_ref01_data = Object.values(setup.data.existing.vcr_repository_permission_list)[0] as any

    // LIST
    const vcr_repository_permission_list_ref01_ent = client.VcrRepositoryPermissionList()
    const vcr_repository_permission_list_ref01_match: any = {}
    vcr_repository_permission_list_ref01_match['id_or_name'] = setup.idmap['id_or_name01']

    const vcr_repository_permission_list_ref01_list = (await vcr_repository_permission_list_ref01_ent.list(vcr_repository_permission_list_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/vcr_repository_permission_list/VcrRepositoryPermissionListTestData.json')
  ...
  let idmap = transform(
    ['vcr_repository_permission_list01','vcr_repository_permission_list02','vcr_repository_permission_list03','repository01','repository02','repository03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
