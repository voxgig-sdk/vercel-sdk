...
describe('ApiAiGatewayEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'api_ai_gateway.' + op, live)) return
    }
    ...
    let api_ai_gateway_ref01_data = Object.values(setup.data.existing.api_ai_gateway)[0] as any

    // LOAD
    const api_ai_gateway_ref01_ent = client.ApiAiGateway()
    const api_ai_gateway_ref01_match_dt0: any = {}
