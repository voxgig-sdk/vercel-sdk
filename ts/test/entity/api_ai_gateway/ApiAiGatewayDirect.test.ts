...
describe('ApiAiGatewayDirect', async () => {
  ...
  test('direct-load-api_ai_gateway', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-api_ai_gateway', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_API_AI_GATEWAY_ENTID']
