...
describe('AiGatewayRuleListDirect', async () => {
  ...
  test('direct-list-ai_gateway_rule_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ai_gateway_rule_list', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_AI_GATEWAY_RULE_LIST_ENTID']
