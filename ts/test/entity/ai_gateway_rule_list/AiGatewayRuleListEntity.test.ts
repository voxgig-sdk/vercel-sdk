...
describe('AiGatewayRuleListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'ai_gateway_rule_list.' + op, live)) return
    }
    ...
    let ai_gateway_rule_list_ref01_data = Object.values(setup.data.existing.ai_gateway_rule_list)[0] as any

    // LIST
    const ai_gateway_rule_list_ref01_ent = client.AiGatewayRuleList()
    const ai_gateway_rule_list_ref01_match: any = {}

    const ai_gateway_rule_list_ref01_list = (await ai_gateway_rule_list_ref01_ent.list(ai_gateway_rule_list_ref01_match)).map((e: any) => e.data())
