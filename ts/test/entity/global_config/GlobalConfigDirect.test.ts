...
describe('GlobalConfigDirect', async () => {
  ...
  test('direct-load-global_config', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-global_config', setup.live)) return
    if (skipIfMissingIds(t, setup, ["edge_config_backup_version01"])) return
  ...
  test('direct-list-global_config', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-global_config', setup.live)) return
    if (skipIfMissingIds(t, setup, ["global_config01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_GLOBAL_CONFIG_ENTID']
