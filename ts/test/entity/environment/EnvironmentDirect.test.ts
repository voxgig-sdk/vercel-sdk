...
describe('EnvironmentDirect', async () => {
  ...
  test('direct-load-environment', async (t: any) => {
    ...
    if (skipIfMissingIds(t, setup, ["environment_slug_or01","project01"])) return
  ...
  test('direct-list-environment', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-environment', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_ENVIRONMENT_ENTID']
