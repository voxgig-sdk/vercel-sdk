...
describe('FlagsSdkKeyWithSecretEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'flags_sdk_key_with_secret.' + op, live)) return
    }
    ...
    let flags_sdk_key_with_secret_ref01_data = Object.values(setup.data.existing.flags_sdk_key_with_secret)[0] as any

    // UPDATE
    const flags_sdk_key_with_secret_ref01_ent = client.FlagsSdkKeyWithSecret()
    const flags_sdk_key_with_secret_ref01_data_up0: any = {}

    const flags_sdk_key_with_secret_ref01_markdef_up0 = { name: 'createdBy', value: 'Mark01-flags_sdk_key_with_secret_ref01_' + setup.now }
    ;(flags_sdk_key_with_secret_ref01_data_up0 as any)[flags_sdk_key_with_secret_ref01_markdef_up0.name] = flags_sdk_key_with_secret_ref01_markdef_up0.value
    ...
    assert((flags_sdk_key_with_secret_ref01_resdata_up0 as any)[flags_sdk_key_with_secret_ref01_markdef_up0.name] === flags_sdk_key_with_secret_ref01_markdef_up0.value)
