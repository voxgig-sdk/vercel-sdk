...
describe('MarketplaceDirect', async () => {
  ...
  test('direct-load-marketplace', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-marketplace', setup.live)) return
    if (skipIfMissingIds(t, setup, ["installation01","invoice01"])) return
  ...
  test('direct-list-marketplace', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-marketplace', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_MARKETPLACE_ENTID']
