...
describe('ChecksV2Direct', async () => {
  ...
  test('direct-load-checks_v2', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-checks_v2', setup.live)) return
    if (skipIfMissingIds(t, setup, ["check_run01","deployment01"])) return
  ...
  test('direct-list-checks_v2', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-checks_v2', setup.live)) return
    if (skipIfMissingIds(t, setup, ["check01","project01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_CHECKS_V2_ENTID']
