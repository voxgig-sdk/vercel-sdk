...
describe('VcrRepositoryListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'vcr_repository_list.' + op, live)) return
    }
    ...
    let vcr_repository_list_ref01_data = Object.values(setup.data.existing.vcr_repository_list)[0] as any

    // LIST
    const vcr_repository_list_ref01_ent = client.VcrRepositoryList()
    const vcr_repository_list_ref01_match: any = {}

    const vcr_repository_list_ref01_list = (await vcr_repository_list_ref01_ent.list(vcr_repository_list_ref01_match)).map((e: any) => e.data())
