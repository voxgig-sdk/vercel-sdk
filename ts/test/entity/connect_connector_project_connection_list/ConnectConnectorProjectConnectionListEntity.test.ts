...
describe('ConnectConnectorProjectConnectionListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'connect_connector_project_connection_list.' + op, live)) return
    }
    ...
    let connect_connector_project_connection_list_ref01_data = Object.values(setup.data.existing.connect_connector_project_connection_list)[0] as any

    // LIST
    const connect_connector_project_connection_list_ref01_ent = client.ConnectConnectorProjectConnectionList()
    const connect_connector_project_connection_list_ref01_match: any = {}
    connect_connector_project_connection_list_ref01_match['connector_id'] = setup.idmap['connector01']

    const connect_connector_project_connection_list_ref01_list = (await connect_connector_project_connection_list_ref01_ent.list(connect_connector_project_connection_list_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/connect_connector_project_connection_list/ConnectConnectorProjectConnectionListTestData.json')
