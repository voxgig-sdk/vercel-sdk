...
describe('ConnectConnectorProjectConnectionListDirect', async () => {
  ...
  test('direct-list-connect_connector_project_connection_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-connect_connector_project_connection_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["connector01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_CONNECT_CONNECTOR_PROJECT_CONNECTION_LIST_ENTID']
