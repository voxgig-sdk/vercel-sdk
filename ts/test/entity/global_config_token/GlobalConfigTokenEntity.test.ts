...
describe('GlobalConfigTokenEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'global_config_token.' + op, live)) return
    }
    ...
    let global_config_token_ref01_data = Object.values(setup.data.existing.global_config_token)[0] as any

    // LOAD
    const global_config_token_ref01_ent = client.GlobalConfigToken()
    const global_config_token_ref01_match_dt0: any = {}
