...
describe('GlobalConfigTokenDirect', async () => {
  ...
  test('direct-load-global_config_token', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-global_config_token', setup.live)) return
    if (skipIfMissingIds(t, setup, ["global_config_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_GLOBAL_CONFIG_TOKEN_ENTID']
