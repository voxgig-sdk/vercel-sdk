...
describe('NetworkingEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let networking_ref01_data = Object.values(setup.data.existing.networking)[0] as any

    // UPDATE
    const networking_ref01_ent = client.Networking()
    const networking_ref01_data_up0: any = {}

    const networking_ref01_resdata_up0 = (await networking_ref01_ent.update(networking_ref01_data_up0)).data()
    assert(null != networking_ref01_resdata_up0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/networking/NetworkingTestData.json')
