...
describe('GlobalConfigItemDirect', async () => {
  ...
  test('direct-load-global_config_item', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-global_config_item', setup.live)) return
  ...
  test('direct-list-global_config_item', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-global_config_item', setup.live)) return
    if (skipIfMissingIds(t, setup, ["global_config_item01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_GLOBAL_CONFIG_ITEM_ENTID']
