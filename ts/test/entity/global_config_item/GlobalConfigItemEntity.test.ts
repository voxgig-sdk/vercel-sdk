...
describe('GlobalConfigItemEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'global_config_item.' + op, live)) return
    }
    ...
    let global_config_item_ref01_data = Object.values(setup.data.existing.global_config_item)[0] as any

    // LIST
    const global_config_item_ref01_ent = client.GlobalConfigItem()
    const global_config_item_ref01_match: any = {}
    global_config_item_ref01_match['edge_config_id'] = setup.idmap['edge_config01']

    const global_config_item_ref01_list = (await global_config_item_ref01_ent.list(global_config_item_ref01_match)).map((e: any) => e.data())


    // LOAD
    const global_config_item_ref01_match_dt0: any = {}
