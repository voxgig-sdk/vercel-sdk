...
describe('CertEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    assert(null != cert_ref01_data.id)


    // LIST
    const cert_ref01_match: any = {}

    const cert_ref01_list = (await cert_ref01_ent.list(cert_ref01_match)).map((e: any) => e.data())

    assert(!isempty(select(cert_ref01_list, { id: cert_ref01_data.id })))


    // UPDATE
    const cert_ref01_data_up0: any = {}
    cert_ref01_data_up0.id = cert_ref01_data.id

    const cert_ref01_markdef_up0 = { name: 'ca', value: 'Mark01-cert_ref01_' + setup.now }
    ;(cert_ref01_data_up0 as any)[cert_ref01_markdef_up0.name] = cert_ref01_markdef_up0.value

    const cert_ref01_resdata_up0 = (await cert_ref01_ent.update(cert_ref01_data_up0)).data()
    assert(cert_ref01_resdata_up0.id === cert_ref01_data_up0.id)

    assert((cert_ref01_resdata_up0 as any)[cert_ref01_markdef_up0.name] === cert_ref01_markdef_up0.value)


    // LOAD
    const cert_ref01_match_dt0: any = {}
    cert_ref01_match_dt0.id = cert_ref01_data.id
    const cert_ref01_data_dt0 = (await cert_ref01_ent.load(cert_ref01_match_dt0)).data()
    assert(cert_ref01_data_dt0.id === cert_ref01_data.id)


    // REMOVE
    const cert_ref01_match_rm0: any = { id: cert_ref01_data.id }
    await cert_ref01_ent.remove(cert_ref01_match_rm0)
  

    // LIST
    const cert_ref01_match_rt0: any = {}

    const cert_ref01_list_rt0 = (await cert_ref01_ent.list(cert_ref01_match_rt0)).map((e: any) => e.data())

    assert(isempty(select(cert_ref01_list_rt0, { id: cert_ref01_data.id })))
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/cert/CertTestData.json')
