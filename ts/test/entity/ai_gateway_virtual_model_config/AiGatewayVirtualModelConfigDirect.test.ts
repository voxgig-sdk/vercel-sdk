...
describe('AiGatewayVirtualModelConfigDirect', async () => {
  ...
  test('direct-load-ai_gateway_virtual_model_config', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ai_gateway_virtual_model_config', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_AI_GATEWAY_VIRTUAL_MODEL_CONFIG_ENTID']
