...
describe('ConnectProjectConnectionDirect', async () => {
  ...
  test('direct-load-connect_project_connection', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-connect_project_connection', setup.live)) return
    if (skipIfMissingIds(t, setup, ["connector_id01","project_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_CONNECT_PROJECT_CONNECTION_ENTID']
