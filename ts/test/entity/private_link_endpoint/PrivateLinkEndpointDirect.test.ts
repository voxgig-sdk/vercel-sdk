...
describe('PrivateLinkEndpointDirect', async () => {
  ...
  test('direct-load-private_link_endpoint', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-private_link_endpoint', setup.live)) return
  ...
  test('direct-list-private_link_endpoint', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-private_link_endpoint', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['VERCEL_TEST_PRIVATE_LINK_ENDPOINT_ENTID']
