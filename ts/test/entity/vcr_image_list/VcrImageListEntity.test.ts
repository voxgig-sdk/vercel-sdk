...
describe('VcrImageListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let vcr_image_list_ref01_data = Object.values(setup.data.existing.vcr_image_list)[0] as any

    // LIST
    const vcr_image_list_ref01_ent = client.VcrImageList()
    const vcr_image_list_ref01_match: any = {}
    vcr_image_list_ref01_match['id_or_name'] = setup.idmap['id_or_name01']

    const vcr_image_list_ref01_list = (await vcr_image_list_ref01_ent.list(vcr_image_list_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/vcr_image_list/VcrImageListTestData.json')
  ...
  let idmap = transform(
    ['vcr_image_list01','vcr_image_list02','vcr_image_list03','repository01','repository02','repository03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
