...
`list()` resolves to an array of AccessGroup ENTITIES — every operation
...
```ts
// Create — returns the created AccessGroup ENTITY (.data() for the record)
...
// Remove
await client.AccessGroup().remove({
  id: created.data().id!,
})
```
...
| `id` | `string` |  |
| `inferenceRegion` | `Record<string, any>` | Region pinned on the VMC for system-credential routing (alias/router only). |
...
| `providerOptions` | `Record<string, any>` | Arbitrary per-provider AI SDK options, keyed by gateway provider slug. |
| `providerOrder` | `any[]` | Ordered list of providers to try as fallbacks on failure. |
| `providerTimeouts` | `Record<string, any>` | Per-request provider timeouts in ms, keyed by provider slug for BYOK credentials. |
...
| `creator` | `Record<string, any>` | Information of the user who created the alias |
| `deletedAt` | `number` | The date when the alias was deleted in milliseconds since the UNIX epoch |
| `deployment` | `Record<string, any>` | A map with the deployment ID, URL and metadata |
| `deploymentId` | `string` | The deployment ID |
| `id` | `string` |  |
| `microfrontends` | `Record<string, any>` | The microfrontends for the alias including the routing configuration |
...
| `protectionBypass` | `Record<string, any>` | The protection bypass for the alias |
...
| `aiGatewayQuota` | `Record<string, any>` | Optional AI Gateway quota configuration for the API key. |
...
| `metadata` | `Record<string, any>` | Generic metadata attached to the API key. |
...
| `quota` | `Record<string, any>` | AI Gateway quota associated with an API key. |
...
| `overwrite` | `boolean` |  |
| `projectId` | `string` |  |
| `redirect` | `Record<string, any>` | The redirect object to edit. |
...
| `output` | `Record<string, any>` | The results of the check Run |
| `ownerId` | `string` |  |
...
| `claims` | `Record<string, any>` | Claims extracted from the provider's tokens per the connector's `ForwardedClaims` allow-list. |
...
| `metadata` | `Record<string, any>` | Driver-specific metadata (e.g., botUserId for Slack). |
...
| `appTokens` | `Record<string, any>` | App-token capabilities and known grants for the connector. |
...
| `connector` | `Record<string, any>` | Updated connector. |
...
| `managed` | `Record<string, any>` | Managed connector metadata exposed without leaking the manager connector or installation identifiers. |
| `name` | `string` | Connector name within the owning team. |
| `params` | `Record<string, any>` | Values for the selected connection method's template fields. |
| `projectId` | `string` | Project to connect during creation. |
| `reconsentNeeded` | `Record<string, any>` | Present when affected users must authorize the connector's new permissions. |
...
| `serviceSync` | `Record<string, any>` | Result of synchronizing the change with the external service. |
...
| `triggers` | `Record<string, any>` | Incoming trigger configuration for the connector. |
...
| `userTokens` | `Record<string, any>` | User-token capabilities and known grants for the connector. |
...
| `project` | `Record<string, any>` | Vercel project connected to the connector. |
...
| --- | --- | --- |
| `aliasAssigned` | `any` |  |
| `aliasError` | `Record<string, any>` | An error object in case aliasing of the deployment failed. |
| `attribution` | `Record<string, any>` | Commit attribution metadata |
...
| `checks` | `Record<string, any>` | Detailed information about v2 deployment checks. |
...
| `createdAt` | `number` |  |
| `creator` | `Record<string, any>` | Metadata information of the deployment creator. |
| `customEnvironment` | `Record<string, any>` | The custom environment used for this deployment, if any |
...
| `gitMetadata` | `Record<string, any>` | Populates initial git metadata for different git providers. |
...
| `meta` | `Record<string, any>` | An object containing the deployment's metadata. |
...
| `platform` | `Record<string, any>` | Metadata about the source platform that triggered the deployment. |
| `prebuilt` | `boolean` |  |
...
| `projectSettings` | `Record<string, any>` | Project settings that will be applied to the deployment. |
...
| `seatBlock` | `Record<string, any>` | NSNB Blocked metadata |
...
| `creator` | `Record<string, any>` | An object containing information of the domain creator, including the user's id, username, and email. |
...
| `available` | `boolean` |  |
| `contactInformation` | `Record<string, any>` | The contact information for the domain. |
...
| `updates` | `Record<string, any>` | An object where each key is an environment variable ID (not the key name) and the value is the update to apply |
...
| Field | Type | Description |
| --- | --- | --- |
| `branchMatcher` | `Record<string, any>` | Configuration for matching git branches to this environment |
...
| `data` | `Record<string, any>` | The data of the segment |
| `description` | `string` | A description of the flag |
| `environments` | `Record<string, any>` | The configuration for the flag in different environments |
| `flagId` | `string` |  |
| `flags` | `any[]` |  |
| `hint` | `string` |  |
| `id` | `string` |  |
...
| `transfer` | `Record<string, any>` | Keeps track of the current state of the Global Config while it gets transferred. |
| `updatedAt` | `number` |  |
...
| `claims` | `Record<string, any>` | The claims to include in the token. |
| `claimsSchema` | `Record<string, any>` | A JSON Schema used to validate the resolved token claims when signing tokens for this issuer. |
| `createdAt` | `string` |  |
| `environments` | `any[]` | The environments for the project grant policy. |
| `headers` | `Record<string, any>` | Additional headers to include in the token. |
| `id` | `string` |  |
...
| `signingKeys` | `any[]` |  |
| `token` | `string` |  |
| `tokenClaims` | `Record<string, any>` | The claims that KMS should include in signed JWTs for this policy. |
...
| `headers` | `Record<string, any>` | Headers to be sent together with the request |
...
| `metadata` | `Record<string, any>` | The configured metadata for the resource as defined by its product's Metadata Schema |
| `name` | `string` | The name of the resource as it is recorded in Vercel |
| `notification` | `Record<string, any>` | The notification, if set, displayed to the user when viewing the resource in Vercel |
| `origin` | `string` |  |
| `ownership` | `string` |  |
...
| `period` | `Record<string, any>` | Subscription period for this billing cycle. |
| `productId` | `string` | The ID of the product the resource is derived from |
| `protocolSettings` | `Record<string, any>` | Any settings provided for the resource to support its product's protocols |
...
| `options` | `Record<string, any>` | Optional configuration options for the microfrontend. |
...
| `hostedZones` | `Record<string, any>` | Metadata about any AWS Route53 Hosted Zones associated with the Network. |
| `id` | `string` | The unique identifier of the Network. |
| `name` | `string` | The name of the network. |
| `peeringConnections` | `Record<string, any>` | Metadata about any AWS Route53 Hosted Zones associated with the Network. |
| `projects` | `Record<string, any>` | Metadata about any projects associated with the Network. |
...
| `integrations` | `any[]` |  |
| `internalContentHint` | `Record<string, any>` | Similar to `contentHints`, but should not be exposed to the user. |
| `internalRoutes` | `any[]` |  |
| `ipBuckets` | `any[]` |  |
| `jobs` | `Record<string, any>` |  |
...
| `sandbox` | `Record<string, any>` | Specifies the default region and failover regions for sandboxes created in the project |
| `security` | `Record<string, any>` |  |
...
| `position` | `Record<string, any>` | Controls where the route is inserted. |
| `prompt` | `string` |  |
| `restore` | `boolean` | If true, restores the staged route to the value in the production version. |
| `route` | `Record<string, any>` | The full route object to replace the existing route with |
| `routes` | `any[]` |  |
| `ruleCount` | `number` | The number of routing rules in this version. |
| `s3Key` | `string` | The S3 key where the routing rules are stored. |
| `version` | `Record<string, any>` | A version of routing rules stored in S3. |
...
| `granularity` | `Record<string, any>` | Time bucket size |
...
| `scope` | `Record<string, any>` | Owner or project scope for the query |
...
| Field | Type | Description |
| --- | --- | --- |
| `activeStage` | `Record<string, any>` | The currently active stage, null if the rollout is aborted |
| `advancementType` | `string` | The advancement type of the rolling release |
| `canaryDeployment` | `Record<string, any>` | The canary deployment being rolled out |
...
| `currentDeployment` | `Record<string, any>` | The current deployment receiving production traffic |
| `nextStage` | `Record<string, any>` | The next stage to be activated, null if not in ACTIVE state |
...
| `env` | `Record<string, any>` | Additional environment variables to set for this command. |
...
| `keepLastSnapshots` | `Record<string, any>` | Protect the N most recent snapshots with different expiration/deletion behavior. |
...
| `mounts` | `Record<string, any>` | List of drives to mount to the sandbox at the provided path. |
...
| `resources` | `Record<string, any>` | Resources to define the VM |
| `resumed` | `boolean` |  |
| `routes` | `any[]` |  |
| `runtime` | `string` | The runtime environment for the sandbox. |
| `sandbox` | `Record<string, any>` | This object contains information related to a Vercel NamedSandbox. |
| `session` | `Record<string, any>` | This object contains information related to a Vercel Sandbox Session. |
...
| `tags` | `Record<string, any>` | Key-value tags to associate with the sandbox. |
...
| `conditions` | `any[]` |  |
| `count` | `number` |  |
| `crs` | `Record<string, any>` | Custom Ruleset |
| `description` | `string` |  |
| `domain` | `string` |  |
| `endTime` | `string` |  |
...
| `attribution` | `Record<string, any>` | Attribution information for the session or current page |
| `avatar` | `string` | The ID of the file used as avatar for this Team. |
| `billing` | `Record<string, any>` | The team's billing plan. |
| `bitbucket` | `Record<string, any>` | Map of the connected Bitbucket account. |
...
| `defaultDeploymentProtection` | `Record<string, any>` | Default deployment protection for this team null indicates protection is disabled |
| `defaultExpirationSettings` | `Record<string, any>` | Default deployment expiration settings for this team |
| `defaultPassport` | `Record<string, any>` | Default Passport configuration for new projects in this team. |
| `defaultProjectJobs` | `Record<string, any>` | Default job configuration applied to new projects created in this team. |
| `defaultRoles` | `Record<string, any>` | Default roles for the team. |
| `deploymentPolicy` | `Record<string, any>` | Composable deployment-time policy for the team. |
...
| `github` | `Record<string, any>` | Map of the connected GitHub account. |
| `gitlab` | `Record<string, any>` | Map of the connected GitLab account. |
...
| `ipBuckets` | `any[]` |  |
| `joinedFrom` | `Record<string, any>` | A map that describes the origin from where the user joined. |
| `membership` | `Record<string, any>` | The membership of the authenticated User in relation to the Team. |
| `name` | `string` | Name associated with the Team account, or `null` if none has been provided. |
| `nsnbConfig` | `Record<string, any>` | NSNB configuration for the team. |
| `orgRootTeamId` | `string` | Best-effort ID of the organization’s root billing team. |
| `pagination` | `Record<string, any>` | This object contains information related to the pagination of the current request, including the necessary parameters to get the next or previous page of data. |
...
| `remoteCaching` | `Record<string, any>` | Is remote caching enabled for this team |
...
| `resourceConfig` | `Record<string, any>` | Resource configuration for the team. |
| `role` | `string` | The role in the team of the member. |
| `saml` | `Record<string, any>` | When "Single Sign-On (SAML)" is configured, this object contains information regarding the configuration of the Identity Provider (IdP). |
...
| `strictConnectors` | `Record<string, any>` | When enabled, creating and managing connectors requires Owner role or the ConnectorManager permission. |
| `strictDeploymentProtectionSettings` | `Record<string, any>` | When enabled, deployment protection settings require stricter permissions (owner-only). |
| `strictPasswordProtectionSettings` | `Record<string, any>` | When enabled, adding, changing, or removing project password protection requires Owner role. |
| `strictShareableLinks` | `Record<string, any>` | When enabled, creating shareable links requires Owner role. |
...
| `user` | `Record<string, any>` | Metadata for {@link userId}. |
