...
func TestKmsDirect(t *testing.T) {
	t.Run("direct-list-kms", func(t *testing.T) {
		setup := kmsDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-kms", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-kms", func(t *testing.T) {
		setup := kmsDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-kms", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			params["issuer_id"] = setup.idmap["issuer01"]
		} else {
			params["issuer_id"] = "direct01"
		}

		result, err := client.Direct(map[string]any{
			"path":   "v1/kms/issuers/{issuer_id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type kmsDirectSetupResult struct {
...
func kmsDirectSetup(mockres any) *kmsDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_KMS_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_KMS_ENTID"]; ok {
		...
		return &kmsDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &kmsDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
