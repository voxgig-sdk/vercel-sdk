...
import (
	...
	omni "github.com/voxgig-sdk/vercel-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VercelError); is {
				out := map[string]any{
					"name":    "VercelError",
...
func omniCtx(arg any, client *sdk.VercelSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VercelSDK); is {
			client = live
