...
func TestVcrDirect(t *testing.T) {
	t.Run("direct-list-vcr", func(t *testing.T) {
		setup := vcrDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-vcr", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"id_or_name01"} {
		...
		result, err := client.Direct(map[string]any{
			"path":   "v1/vcr/repository/{id_or_name}/tags",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-vcr", func(t *testing.T) {
		setup := vcrDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-vcr", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["id_or_name"] = setup.idmap["id_or_name01"]
			...
			params["image_id_or_digest"] = setup.idmap["image_id_or_digest01"]
			params["repository_id"] = setup.idmap["repository01"]
		} else {
			params["image_id_or_digest"] = "direct01"
			params["repository_id"] = "direct02"
		}

		result, err := client.Direct(map[string]any{
			"path":   "v1/vcr/repository/{repository_id}/images/{image_id_or_digest}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type vcrDirectSetupResult struct {
...
func vcrDirectSetup(mockres any) *vcrDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_VCR_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_VCR_ENTID"]; ok {
		...
		return &vcrDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &vcrDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
