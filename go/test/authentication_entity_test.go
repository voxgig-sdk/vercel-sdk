...
func TestAuthenticationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_AUTHENTICATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		// REMOVE
		authenticationRef01MatchRm0 := map[string]any{
			"id": authenticationRef01Data["id"],
		}
		_, err = authenticationRef01Ent.Remove(authenticationRef01MatchRm0, nil)
		if err != nil {
...
func authenticationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"authentication01", "authentication02", "authentication03", "token01", "token02", "token03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("VERCEL_TEST_AUTHENTICATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VERCEL_TEST_AUTHENTICATION_ENTID"])
