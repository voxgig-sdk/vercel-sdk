...
func TestCertDirect(t *testing.T) {
	t.Run("direct-list-cert", func(t *testing.T) {
		setup := certDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-cert", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-cert", func(t *testing.T) {
		setup := certDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-cert", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type certDirectSetupResult struct {
...
func certDirectSetup(mockres any) *certDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_CERT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_CERT_ENTID"]; ok {
		...
		return &certDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &certDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
