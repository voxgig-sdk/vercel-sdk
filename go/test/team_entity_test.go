...
func TestTeamEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_TEAM_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		teamRef01MarkdefUp0Name := "avatar"
...
func teamBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"team01", "team02", "team03", "invite01", "invite02", "invite03", "member01", "member02", "member03", "microfrontend01", "microfrontend02", "microfrontend03", "request01", "request02", "request03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("VERCEL_TEST_TEAM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VERCEL_TEST_TEAM_ENTID"])
