...
func TestProjectRouteDirect(t *testing.T) {
	t.Run("direct-list-project_route", func(t *testing.T) {
		setup := project_routeDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-project_route", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "v1/projects/{project_id}/routes/versions",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-project_route", func(t *testing.T) {
		setup := project_routeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-project_route", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type project_routeDirectSetupResult struct {
...
func project_routeDirectSetup(mockres any) *project_routeDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_PROJECT_ROUTE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_PROJECT_ROUTE_ENTID"]; ok {
		...
		return &project_routeDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &project_routeDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
