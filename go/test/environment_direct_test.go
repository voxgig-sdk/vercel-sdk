...
func TestEnvironmentDirect(t *testing.T) {
	t.Run("direct-list-environment", func(t *testing.T) {
		setup := environmentDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-environment", _mode); _shouldSkip {
			if _reason == "" {
...
func environmentDirectSetup(mockres any) *environmentDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"VERCEL_TEST_ENVIRONMENT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_ENVIRONMENT_ENTID"]; ok {
