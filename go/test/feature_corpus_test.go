...
//
// NewVercelSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.VercelSDK {
	res, _ := kase["res"].([]any)
...
func fcCandidates(client *sdk.VercelSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.VercelSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.VercelSDK, name string) any {
