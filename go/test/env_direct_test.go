...
func TestEnvDirect(t *testing.T) {
	t.Run("direct-list-env", func(t *testing.T) {
		setup := envDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-env", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-env", func(t *testing.T) {
		setup := envDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-env", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type envDirectSetupResult struct {
...
func envDirectSetup(mockres any) *envDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_ENV_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_ENV_ENTID"]; ok {
		...
		return &envDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &envDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
