...
func TestMicrofrontendDirect(t *testing.T) {
	t.Run("direct-list-microfrontend", func(t *testing.T) {
		setup := microfrontendDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-microfrontend", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "v1/microfrontends/groups/{group_id}/projects",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-microfrontend", func(t *testing.T) {
		setup := microfrontendDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-microfrontend", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "v1/microfrontends/{id}/config",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type microfrontendDirectSetupResult struct {
...
func microfrontendDirectSetup(mockres any) *microfrontendDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_MICROFRONTEND_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_MICROFRONTEND_ENTID"]; ok {
		...
		return &microfrontendDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &microfrontendDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
