...
func TestSecurityDirect(t *testing.T) {
	t.Run("direct-list-security", func(t *testing.T) {
		setup := securityDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-security", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-security", func(t *testing.T) {
		setup := securityDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-security", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			params["config_version"] = setup.idmap["config_version01"]
		} else {
			params["config_version"] = "direct01"
		}

		result, err := client.Direct(map[string]any{
			"path":   "v1/security/firewall/config/{config_version}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type securityDirectSetupResult struct {
...
func securityDirectSetup(mockres any) *securityDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"VERCEL_TEST_SECURITY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["VERCEL_TEST_SECURITY_ENTID"]; ok {
		...
		return &securityDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &securityDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
