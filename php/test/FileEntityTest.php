...
class FileEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_FILE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $file_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.file")));
        $file_ref01_data = null;
        if (count($file_ref01_data_raw) > 0) {
            $file_ref01_data = Helpers::to_map($file_ref01_data_raw[0][1]);
        }
...
function file_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["file01", "file02", "file03", "deployment01", "deployment02", "deployment03"] as $k) {
    ...
    $entid_env_raw = getenv("VERCEL_TEST_FILE_ENTID");
