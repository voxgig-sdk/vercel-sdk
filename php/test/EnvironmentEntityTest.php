...
class EnvironmentEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Environment(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Environment(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_ENVIRONMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $environment_ref01_data["env_id"] = $setup["idmap"]["env01"];
        $environment_ref01_data["id_or_name"] = $setup["idmap"]["id_or_name01"];
        ...
        // LIST
        $environment_ref01_match = [
            "id_or_name" => $setup["idmap"]["id_or_name01"],
        ];

        $environment_ref01_list_result = $environment_ref01_ent->list($environment_ref01_match, null);
        $this->assertIsArray($environment_ref01_list_result);

        $found_item = sdk_select(
            Runner::entity_list_to_data($environment_ref01_list_result),
        ...
        $environment_ref01_markdef_up0_name = "copyEnvVarsFrom";
        ...
        // LIST
        $environment_ref01_match_rt0 = [
            "id_or_name" => $setup["idmap"]["id_or_name01"],
        ];

        $environment_ref01_list_rt0_result = $environment_ref01_ent->list($environment_ref01_match_rt0, null);
        $this->assertIsArray($environment_ref01_list_rt0_result);

        $not_found_item = sdk_select(
            Runner::entity_list_to_data($environment_ref01_list_rt0_result),
            ["id" => $environment_ref01_data["id"]]);
...
function environment_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["environment01", "environment02", "environment03", "project01", "project02", "project03", "env01", "env02", "env03", "unlink01", "unlink02", "unlink03", "custom_environment01", "custom_environment02", "custom_environment03", "id_or_name01"] as $k) {
    ...
    $entid_env_raw = getenv("VERCEL_TEST_ENVIRONMENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["VERCEL_TEST_ENVIRONMENT_ENTID"]);
