...
class AuthenticationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_AUTHENTICATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        // REMOVE
        $authentication_ref01_match_rm0 = [
            "id" => $authentication_ref01_data["id"],
        ];
        $authentication_ref01_ent->remove($authentication_ref01_match_rm0, null);
...
function authentication_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["authentication01", "authentication02", "authentication03", "token01", "token02", "token03"] as $k) {
    ...
    $entid_env_raw = getenv("VERCEL_TEST_AUTHENTICATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["VERCEL_TEST_AUTHENTICATION_ENTID"]);
