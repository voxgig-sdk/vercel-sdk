...
class FeatureFlagEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->FeatureFlag(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->FeatureFlag(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_FEATURE_FLAG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $feature_flag_ref01_data = null;
        if (count($feature_flag_ref01_data_raw) > 0) {
        ...
        $feature_flag_ref01_list_result = $feature_flag_ref01_ent->list($feature_flag_ref01_match, null);
        $this->assertIsArray($feature_flag_ref01_list_result);
        ...
        $feature_flag_ref01_markdef_up0_value = "Mark01-feature_flag_ref01_" . $setup["now"];
        $feature_flag_ref01_data_up0_up[$feature_flag_ref01_markdef_up0_name] = $feature_flag_ref01_markdef_up0_value;

        $feature_flag_ref01_resdata_up0_result = $feature_flag_ref01_ent->update($feature_flag_ref01_data_up0_up, null);
        $feature_flag_ref01_resdata_up0 = Helpers::to_map(is_object($feature_flag_ref01_resdata_up0_result) && method_exists($feature_flag_ref01_resdata_up0_result, 'data_get') ? $feature_flag_ref01_resdata_up0_result->data_get() : $feature_flag_ref01_resdata_up0_result);
        $this->assertNotNull($feature_flag_ref01_resdata_up0);
        $this->assertEquals($feature_flag_ref01_resdata_up0["id"], $feature_flag_ref01_data_up0_up["id"]);
        $this->assertEquals($feature_flag_ref01_resdata_up0[$feature_flag_ref01_markdef_up0_name], $feature_flag_ref01_markdef_up0_value);
...
function feature_flag_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["feature_flag01", "feature_flag02", "feature_flag03", "deployment01", "deployment02", "deployment03", "project01", "project02", "project03", "team01", "team02", "team03", "flag01", "flag02", "flag03", "sdk_key01", "sdk_key02", "sdk_key03", "segment01", "segment02", "segment03"] as $k) {
    ...
    $entid_env_raw = getenv("VERCEL_TEST_FEATURE_FLAG_ENTID");
