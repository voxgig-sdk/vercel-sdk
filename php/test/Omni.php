...
//    only - a typed VercelContext there would make every ctx
...
final class VercelOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new VercelContext($clean, null);
    ...
    public static function err_from_map($m): ?VercelError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new VercelError(is_string($code) ? $code : '', $msg);
    }
