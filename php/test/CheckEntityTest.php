...
class CheckEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_CHECK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function check_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["check01", "check02", "check03", "deployment01", "deployment02", "deployment03", "project01", "project02", "project03", "project_or_name01"] as $k) {
    ...
    $entid_env_raw = getenv("VERCEL_TEST_CHECK_ENTID");
