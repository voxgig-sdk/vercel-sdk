...
class RecordEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VERCEL_TEST_RECORD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $record_ref01_match_dt0 = [
            "id" => $record_ref01_data["id"],
        ];
        $record_ref01_data_dt0_loaded = $record_ref01_ent->load($record_ref01_match_dt0, null);
        $record_ref01_data_dt0_load_result = Helpers::to_map(is_object($record_ref01_data_dt0_loaded) && method_exists($record_ref01_data_dt0_loaded, 'data_get') ? $record_ref01_data_dt0_loaded->data_get() : $record_ref01_data_dt0_loaded);
        $this->assertNotNull($record_ref01_data_dt0_load_result);
        $this->assertEquals($record_ref01_data_dt0_load_result["id"], $record_ref01_data["id"]);
...
function record_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("VERCEL_TEST_RECORD_ENTID");
