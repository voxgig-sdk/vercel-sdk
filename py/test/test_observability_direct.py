...
class TestObservabilityDirect:

    def test_should_direct_list_observability(self):
        setup = _observability_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-observability", "live" if setup["live"] else "unit")
        if _skip:
...
def _observability_direct_setup(mockres):
