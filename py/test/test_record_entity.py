    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VERCEL_TEST_RECORD_ENTID JSON to run live")
        client = setup["client"]
        ...
        record_ref01_data_dt0_loaded = record_ref01_ent.load(record_ref01_match_dt0, None)
        record_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(record_ref01_data_dt0_loaded))
        assert record_ref01_data_dt0_load_result is not None
        assert record_ref01_data_dt0_load_result["id"] == record_ref01_data["id"]
...
def _record_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("VERCEL_TEST_RECORD_ENTID"))
