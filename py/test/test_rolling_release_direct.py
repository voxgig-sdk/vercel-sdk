...
class TestRollingReleaseDirect:

    def test_should_direct_load_rolling_release(self):
        setup = _rolling_release_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-rolling_release", "live" if setup["live"] else "unit")
        if _skip:
...
def _rolling_release_direct_setup(mockres):
