...
class TestAccessGroupDirect:

    def test_should_direct_list_access_group(self):
        setup = _access_group_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-access_group", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_access_group(self):
        setup = _access_group_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-access_group", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _access_group_direct_setup(mockres):
