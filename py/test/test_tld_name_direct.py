...
class TestTldNameDirect:

    def test_should_direct_list_tld_name(self):
        setup = _tld_name_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tld_name", "live" if setup["live"] else "unit")
        if _skip:
...
def _tld_name_direct_setup(mockres):
