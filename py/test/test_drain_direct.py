...
class TestDrainDirect:
    ...
    def test_should_direct_load_drain(self):
        setup = _drain_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-drain", "live" if setup["live"] else "unit")
        if _skip:
...
def _drain_direct_setup(mockres):
