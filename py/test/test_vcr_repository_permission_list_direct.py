...
class TestVcrRepositoryPermissionListDirect:

    def test_should_direct_list_vcr_repository_permission_list(self):
        setup = _vcr_repository_permission_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-vcr_repository_permission_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _vcr_repository_permission_list_direct_setup(mockres):
