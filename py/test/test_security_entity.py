...
class TestSecurityEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Security(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Security(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VERCEL_TEST_SECURITY_ENTID JSON to run live")
        client = setup["client"]
        ...
        security_ref01_ent.remove(security_ref01_match_rm0, None)
...
def _security_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("VERCEL_TEST_SECURITY_ENTID"))
