...
class TestChecksV2Direct:

    def test_should_direct_list_checks_v2(self):
        setup = _checks_v2_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-checks_v2", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["check01", "project01"]:
    ...
    def test_should_direct_load_checks_v2(self):
        setup = _checks_v2_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-checks_v2", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _checks_v2_direct_setup(mockres):
