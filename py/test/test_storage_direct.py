...
class TestStorageDirect:
    ...
    def test_should_direct_load_storage(self):
        setup = _storage_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-storage", "live" if setup["live"] else "unit")
        if _skip:
...
def _storage_direct_setup(mockres):
