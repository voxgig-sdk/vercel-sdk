...
class TestEnvironmentEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Environment(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Environment(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VERCEL_TEST_ENVIRONMENT_ENTID JSON to run live")
        client = setup["client"]
        ...
        environment_ref01_list_result = environment_ref01_ent.list(environment_ref01_match, None)
        assert isinstance(environment_ref01_list_result, list)
        ...
        environment_ref01_list_rt0_result = environment_ref01_ent.list(environment_ref01_match_rt0, None)
        assert isinstance(environment_ref01_list_rt0_result, list)
...
def _environment_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("VERCEL_TEST_ENVIRONMENT_ENTID"))
