    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VERCEL_TEST_AUTHENTICATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        authentication_ref01_ent.remove(authentication_ref01_match_rm0, None)
...
def _authentication_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "VERCEL_TEST_AUTHENTICATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("VERCEL_TEST_AUTHENTICATION_ENTID"))
