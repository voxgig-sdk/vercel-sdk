    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VERCEL_TEST_NETWORK_ENTID JSON to run live")
        client = setup["client"]
        ...
        network_ref01_data = helpers.to_map(runner.entity_data(network_ref01_ent.create(network_ref01_data, None)))
        assert network_ref01_data is not None
        assert network_ref01_data["id"] is not None
        ...
        network_ref01_markdef_up0_value = "Mark01-network_ref01_" + str(setup["now"])
        network_ref01_data_up0_up[network_ref01_markdef_up0_name] = network_ref01_markdef_up0_value

        network_ref01_resdata_up0 = helpers.to_map(runner.entity_data(network_ref01_ent.update(network_ref01_data_up0_up, None)))
        assert network_ref01_resdata_up0 is not None
        assert network_ref01_resdata_up0["id"] == network_ref01_data_up0_up["id"]
        assert network_ref01_resdata_up0[network_ref01_markdef_up0_name] == network_ref01_markdef_up0_value
        ...
        network_ref01_ent.remove(network_ref01_match_rm0, None)

        # LIST
        network_ref01_match_rt0 = {}

        network_ref01_list_rt0_result = network_ref01_ent.list(network_ref01_match_rt0, None)
        assert isinstance(network_ref01_list_rt0_result, list)
...
def _network_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("VERCEL_TEST_NETWORK_ENTID"))
