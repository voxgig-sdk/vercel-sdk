...
class TestListEventTypeDirect:

    def test_should_direct_list_list_event_type(self):
        setup = _list_event_type_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-list_event_type", "live" if setup["live"] else "unit")
        if _skip:
...
def _list_event_type_direct_setup(mockres):
