...
class RollingReleaseEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RollingReleaseEntity(self._client, opts)
    ...
    def data_get(self) -> RollingRelease:
    ...
    def match_get(self) -> RollingRelease:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RollingRelease:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RollingRelease().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: RollingReleaseCreateData, ctrl=None) -> RollingRelease:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: RollingReleaseUpdateData, ctrl=None) -> RollingRelease:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> RollingRelease:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.RollingRelease().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
