...
class MicrofrontendEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MicrofrontendEntity(self._client, opts)
    ...
    def data_get(self) -> Microfrontend:
    ...
    def match_get(self) -> Microfrontend:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Microfrontend:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Microfrontend().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Microfrontend]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Microfrontend().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: MicrofrontendCreateData, ctrl=None) -> Microfrontend:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
