...
class DomainsRegistrarEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DomainsRegistrarEntity(self._client, opts)
    ...
    def data_get(self) -> DomainsRegistrar:
    ...
    def match_get(self) -> DomainsRegistrar:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DomainsRegistrar:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DomainsRegistrar().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DomainsRegistrarCreateData, ctrl=None) -> DomainsRegistrar:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: DomainsRegistrarUpdateData, ctrl=None) -> DomainsRegistrar:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
